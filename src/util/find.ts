...
function findSystem(
  ...
  const [targetSystems, otherSystems] = partition(
    board,
    (s) => s.id === system.id
  );
...
function largestShipInSystem(system: System, player?: PlayerId): number {
  const { ships = [] } = system;
  const shipsToSearch =
    player === null ? ships : ships.filter((ship) => ship.owner === player);
  let largest = 0;
